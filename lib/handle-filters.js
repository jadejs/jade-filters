...
function handleFilters(ast, filters, options) {
  ...
  walk(ast, function (node) {
     ...
     * @param mergeGlobalOptions Merge local options with global, true by default
     * @return {{}}
     */
    function getAttributes(node, mergeGlobalOptions) {
      var attrs = {};
      ...
      if (mergeGlobalOptions === undefined || mergeGlobalOptions) {
        var globalOptions = options[node.name];
        if (globalOptions) {
          Object.keys(globalOptions).forEach(function (opt) {
            if (!attrs.hasOwnProperty(opt)) {
              attrs[opt] = globalOptions[opt];
            }
          });
        }
      }
